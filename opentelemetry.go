...
// StringToTraceType - given a comma-separated string of types, returns OR'd tracetypes counterpart
func StringToTraceType(typeStr string) TraceType {
...
func ParseSampleRate(s string) (float64, error) {
	...
	if strings.ContainsRune(s, '/') {
		...
		nom, err := strconv.ParseFloat(split[0], 64)
		if err != nil {
			return 0, fmt.Errorf("invalid sample rate (%s)", s)
		}
		det, err := strconv.ParseFloat(split[1], 64)
		if err != nil || det <= 0 {
			return 0, fmt.Errorf("invalid sample rate (%s)", s)
		}
		return nom / det, nil
	...
	if strings.ContainsRune(s, '%') {
		...
		pct, err := strconv.ParseFloat(split[0], 64)
		if err != nil || pct <= 0 {
			return 0, fmt.Errorf("invalid sample rate (%s)", s)
		}
		return pct / 100, nil
