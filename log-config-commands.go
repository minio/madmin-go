...
func (adm *AdminClient) GetLogConfig(ctx context.Context) (*LogRecorderStatus, error) {
	// Execute GET on /minio/admin/v3/logs-config to get log configuration
	resp, err := adm.executeMethod(ctx,
		http.MethodGet,
		requestData{relPath: adminAPIPrefix + "/logs-config"})
...
func (adm *AdminClient) SetLogConfig(ctx context.Context, config *LogRecorderConfig) error {
	...
	reqData := requestData{
		relPath: adminAPIPrefix + "/logs-config",
		content: econfigBytes,
	}

	// Execute PUT on /minio/admin/v3/logs-config to set log configuration
...
func (adm *AdminClient) ResetLogConfig(ctx context.Context) error {
	...
	// Execute DELETE on /minio/admin/v3/logs-config to reset configuration
