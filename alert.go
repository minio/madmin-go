...
// AlertEvent represents a single alert event in the system.
// It captures alert information with contextual metadata including deployment and site information.
...
func (adm AdminClient) GetAlerts(ctx context.Context, opts AlertLogOpts) iter.Seq2[AlertEvent, error] {
	return func(yield func(AlertEvent, error) bool) {
		alertOpts, err := json.Marshal(opts)
		if err != nil {
			yield(AlertEvent{}, err)
			return
		}
		reqData := requestData{
		...
		if resp.StatusCode != http.StatusOK {
			yield(AlertEvent{}, httpRespToErrorResponse(resp))
			return
		}
