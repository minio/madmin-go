...
type QOSConfig struct {
	Version string    `yaml:"version"` // Version of the QoS configuration format
	Rules   []QOSRule `yaml:"rules"`
}

// QOSRule represents a single QoS rule at the bucket prefix or bucket wildcard level.
type QOSRule struct {
	ID           string          `yaml:"id"`
	Label        string          `yaml:"label,omitempty"`
	BucketPrefix string          `yaml:"bucketPrefix"` // includes prefix if any and represents simple wildcard for bucket, prefix
	Requests     []RequestLimits `yaml:"requests,omitempty" json:"requests,omitempty" mapstructure:"requests"`
}

// RequestLimits - define Request Limits
type RequestLimits struct {
	API      string `yaml:"api" json:"api" mapstructure:"api"`
	APILimit `yaml:",inline" mapstructure:",squash"`
}

// APILimit - define API Limit
type APILimit struct {
	Rate  int64  `yaml:"rate" json:"rate" mapstructure:"rate"`    // requests per second for (rps), concurrent requests for (concurrency)
	Burst int64  `yaml:"burst" json:"burst" mapstructure:"burst"` // burst size for the limiter
	Limit string `yaml:"limit" json:"type" mapstructure:"limit"`  // "concurrency" or "rps" limiter
}
